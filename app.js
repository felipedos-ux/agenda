...
const db = {
  ...
  async deleteProject(id) {
    // 1. Delete all tasks associated with this project first (Foreign Key Constraint)
  ...
  async loadSettings() {
    ...
    return {
      notificationSound: data.notification_sound || 'bell',
      defaultCalendarView: data.default_calendar_view || 'month',
      enableNotifications: data.enable_notifications !== false,
      notificationTime: data.notification_time || 15,
  ...
  async loadAllInto(state) {
    const [tasks, exams, sup, far, projects, settings] = await Promise.all([
      this.loadTasks(),
      this.loadExams(),
...
const app = {
  ...
  async init() {
    ...
    // 3. Apply theme from database (overrides localStorage if different)
    if (this.state.settings?.theme) {
      document.documentElement.setAttribute('data-color-scheme', this.state.settings.theme);
      document.getElementById('themeIcon').textContent = this.state.settings.theme === 'dark' ? '☀️' : '🌙';
      localStorage.setItem('agenda_theme', this.state.settings.theme);
    }

    // Apply default calendar view from settings
    if (this.state.settings?.defaultCalendarView) {
      this.state.calendarView = this.state.settings.defaultCalendarView;
    }
  ...
  switchTab(tabName) {
    ...
    localStorage.setItem('agenda_current_tab', tabName);
  ...
  toggleTheme() {
    ...
    // Apply immediately (instant visual feedback)
    html.setAttribute('data-color-scheme', newTheme);
    document.getElementById('themeIcon').textContent = newTheme === 'dark' ? '☀️' : '🌙';

    // Save to localStorage (instant persistence on reload)
    localStorage.setItem('agenda_theme', newTheme);

    // Save to state and database (sync across devices)
    this.state.settings.theme = newTheme;
    db.saveSettings(this.state.settings).catch(console.error);
  },
  ...
  toggleProjectCollapse(projectId) {
    if (!this.state.collapsedProjects) {
      this.state.collapsedProjects = {};
    }

    // true ou undefined = colapsado | false = expandido
    const isCollapsedNow =
      this.state.collapsedProjects[projectId] !== false;

    // inverte o estado
    this.state.collapsedProjects[projectId] = !isCollapsedNow;
  ...
  renderProjects() {
    ...
    if (projects.length === 0) {
      container.innerHTML =
        '<p style="color: var(--color-text-secondary); text-align: center; padding: var(--space-24);">Nenhum projeto criado.</p>';
      return;
    }
    ...
    projects.forEach(project => {
      ...
      // DEFAULT: colapsado (se for undefined, considera colapsado)
      const isCollapsed =
        this.state.collapsedProjects[project.id] !== false;

      html += `
        <div class="project-card ${isCollapsed ? 'project-card--collapsed' : ''}">
          <div class="project-header">
            ...
            <div style="display: flex; gap: var(--space-8); align-items: center;">
              <button
                class="btn btn--sm btn--secondary project-toggle-btn"
                onclick="app.toggleProjectCollapse('${project.id}')"
              >
                ${isCollapsed ? '▸' : '▾'}
              </button>
              <button
                class="btn btn--sm btn--primary"
                onclick="app.openProjectTaskModal('${project.id}')"
              >
                + Tarefa
              </button>
              <button
                class="btn btn--sm btn--secondary"
                onclick="app.openProjectModal('${project.id}')"
              >
                ✏️
              </button>
              <button
      ...
      if (tasks.length === 0) {
        ...
        tasks.forEach(task => {
          ...
          let currentTime = task.timeSpent || 0;
          if (task.isRunning && task.startTime) {
            currentTime += Date.now() - task.startTime;
          }

          // Se estiver rodando e não concluída -> "andamento"
          const stateForBadge =
          ...
          html += `
            <div class="project-task-item status-${stateForBadge}">
              ...
              : ''
                ...
                <button
                  class="btn btn--sm btn--secondary"
                  onclick="app.openProjectTaskModal('${project.id}', '${task.id}')"
                >
                  ✏️
                </button>
                <button
  ...
  saveSettings() {
    const settings = {
      ...
      defaultCalendarView: document.getElementById('defaultCalendarView').value,
    ...
    db.saveSettings(settings)
      .then(() => {
        ...
        // Apply calendar view immediately if changed
        if (this.state.calendarView !== settings.defaultCalendarView) {
          this.state.calendarView = settings.defaultCalendarView;
          this.renderCalendar();
        }
      })
      .catch(error => {
